...
function App() {
  ...
  const showAlert = (newAlert) => {
    // Clear any existing alert immediately before setting a new one
    setAlert(null); // Reset alert state
    setTimeout(() => {
      setAlert(newAlert); // Set new alert after a brief reset
      setTimeout(() => {
        setAlert(null); // Clear alert after 5 seconds
      }, 5000);
    }, 10); // Small delay to ensure reset is processed
  };
